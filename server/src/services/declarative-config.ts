...
function upsertApiKey(db: DatabaseType, input: z.infer<typeof keySchema>): number {
...
function ensureFallbackRow(db: DatabaseType, modelDbId: number, enabled = true, updateExisting = true): void {
...
function registerCustomProvider(db: DatabaseType, input: z.infer<typeof customProviderSchema>): number {
...
function upsertModel(db: DatabaseType, input: z.infer<typeof modelSchema>): void {
  const platform = input.platform.trim();
...
function applyFallback(db: DatabaseType, entries: z.infer<typeof fallbackEntrySchema>[]): number {
