...
export function refreshStatsCache(db: DatabaseType, force = false): void {
...
function getActiveChain(db: DatabaseType): ChainRow[] {
...
function getChainByProfileName(db: DatabaseType, name: string): ChainRow[] | null {
...
function getChainByGlobalSort(db: DatabaseType, globalAxis: string): ChainRow[] {
 ...
 */
function getModelChainRow(db: DatabaseType, modelDbId: number): ChainRow | undefined {
  return db.prepare(`
    SELECT m.id as model_db_id, 0 as priority, 1 as enabled,
           m.platform, m.model_id, m.display_name, m.intelligence_rank,
           m.size_label, m.monthly_token_budget,
