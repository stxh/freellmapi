...
export async function backupDbNow(db: DatabaseType, dbPath = getDefaultDbPath()): Promise<DbBackupResult> {
  const target = backupTarget();
  if (!target) return { ok: true, skipped: 'not configured' };
  if (dbPath === ':memory:') return { ok: true, target, skipped: 'memory database' };
...
export function startDbBackupPump(db: DatabaseType, scheduler: Scheduler, dbPath = getDefaultDbPath()): (() => void) | null {
